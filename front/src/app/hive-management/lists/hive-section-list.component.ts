...
export class HiveSectionListComponent implements OnInit {
  ...
  onDelete(hiveSectionId: number) {
    var hiveSections = this.hiveSections.find(h => h.id == hiveSectionId);
    this.hiveSectionService.setHiveSectionStatus(hiveSectionId, true).subscribe(c => hiveSections.isDeleted = true);
  }

  onUndelete(hiveSectionId: number) {
    var hiveSections = this.hiveSections.find(h => h.id == hiveSectionId);
    this.hiveSectionService.setHiveSectionStatus(hiveSectionId, false).subscribe(c => hiveSections.isDeleted = false);
  }
}
